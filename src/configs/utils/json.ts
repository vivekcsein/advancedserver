...
export const writeDataOnJson = (dir: string, filename: string, newdata: Partial<ProductData>) => {
    const currDir = path.join(__dirname, dir, filename);
    if (fs.existsSync(currDir)) {
...
// update on json data
export const updateDataInJson = (dir: string, filename: string, newdata: Partial<ProductData>, index: number) => {
    const currDir = path.join(__dirname, dir, filename);
    if (fs.existsSync(currDir)) {
...
export const deleteDataInJson = (dir: string, filename: string, index: number) => {
    const currDir = path.join(__dirname, dir, filename);
    if (fs.existsSync(currDir)) {
...
const updateNewData = (data: ProductsData, newData: Partial<ProductData>, index: number): ProductsData => {
...
const deleteData = (data: ProductsData, index: number): ProductsData => {
    const updatedData = { ...data };
    const filteredProducts = updatedData.productsData.filter((_val, i) => i !== index);
