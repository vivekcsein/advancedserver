...
import { readDataFromJson, writeDataOnJson, updateDataInJson, deleteDataInJson } from "../utils/json.js"
...
export const updateProductJson = (newdata: Partial<ProductData>, productIndex: number) => {
    return new Promise((resolve, reject) => {
        try {
            let result = updateDataInJson(dataDir, "products.json", newdata, productIndex);
            resolve(result);
        } catch (error) {
            reject(error);
        }
